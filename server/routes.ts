...
// Configuration for version limits to prevent performance issues
const MAX_VERSIONS_PER_PACKAGE = 5;

// Simple in-memory cache for package data (5 minutes TTL)
const packageCache = new Map<string, { data: PackageAttestation[], timestamp: number }>();
...
async function fetchNpmPackages(username: string): Promise<PackageAttestation[]> {
  try {
    ...
    for (const pkg of data.objects || []) {
      try {
        // Fetch package details for all versions
        const detailResponse = await fetch(`https://registry.npmjs.org/${pkg.package.name}`);
        if (!detailResponse.ok) continue;
        
        const detail = await detailResponse.json();
        
        // Get all versions and sort by publish time (newest first)
        const allVersions = Object.keys(detail.versions || {});
        const versionsWithTime = allVersions
          .map(version => ({
            version,
            publishedAt: detail.time?.[version] || new Date(0).toISOString()
          }))
          .sort((a, b) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime())
          .slice(0, MAX_VERSIONS_PER_PACKAGE);
        
        // Create attestation entries for each version
        for (const versionInfo of versionsWithTime) {
          const version = versionInfo.version;
          
          // Check for npm attestations (SLSA provenance)
          const attestationStatus = await checkNpmAttestation(pkg.package.name, version);
          
          packages.push({
            id: `npm-${pkg.package.name}-${version}`,
            ...
            license: detail.versions?.[version]?.license,
    ...
    for (const packageName of commonPythonPackages) {
      try {
        ...
        if (packageResponse.ok) {
          ...
          // Get all versions and sort by release date (newest first)
          const releases = packageData.releases || {};
          const allVersions = Object.keys(releases);
          
          // Sort versions by upload time (get first file's upload time for each version)
          const versionsWithTime = allVersions
            .map(version => {
              ...
              return {
                version,
                publishedAt: firstFile?.upload_time_iso_8601 || new Date(0).toISOString()
              };
            })
            .filter(v => v.publishedAt !== new Date(0).toISOString()) // Filter out versions without dates
            .sort((a, b) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime())
            .slice(0, MAX_VERSIONS_PER_PACKAGE);
          ...
          for (const versionInfo of versionsWithTime) {
            packages.push({
              id: `pypi-${packageName}-${versionInfo.version}`,
              ...
              packageUrl: `https://pypi.org/project/${packageName}/${versionInfo.version}/`,
  ...
  try {
    ...
    for (const repo of repos) {
      try {
        ...
        const releasesResponse = await fetch(`https://api.github.com/repos/${username}/${repo.name}/releases?per_page=${MAX_VERSIONS_PER_PACKAGE}`, {
          headers: {
            'Authorization': `token ${token}`,
            'Accept': 'application/vnd.github.v3+json',
          },
        ...
        if (releasesResponse.ok) {
          ...
          // Sort releases by published date (newest first) and take up to MAX_VERSIONS_PER_PACKAGE
          const sortedReleases = releases
            .filter((release: any) => !release.draft && !release.prerelease) // Filter out drafts and prereleases
            .sort((a: any, b: any) => new Date(b.published_at || b.created_at).getTime() - new Date(a.published_at || a.created_at).getTime())
            .slice(0, MAX_VERSIONS_PER_PACKAGE);
          
          // Create package attestation entry for each release
          for (const release of sortedReleases) {
            // Check for GitHub attestations (reuse repo-level result for each version)
            const attestationStatus = await checkGitHubAttestation(username, repo.name, release.tag_name);
            
            packages.push({
              id: `github-${repo.name}-${release.tag_name}`,
              packageName: repo.name,
              version: release.tag_name,
              registry: "github",
              publishedAt: release.published_at || release.created_at || repo.updated_at,
              attestationStatus: attestationStatus.status,
              attestationUrl: attestationStatus.url,
              attestationDetails: attestationStatus.details,
              packageUrl: release.html_url || repo.html_url,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/packages/attestations", async (req, res) => {
    try {
      ...
      // Check cache first
      const cacheKey = `packages-${username}`;
      const cached = packageCache.get(cacheKey);
      const now = Date.now();
      
      if (cached && (now - cached.timestamp) < CACHE_TTL) {
        console.log(`Returning cached package attestations for user: ${username}`);
      ...
      // Cache the results
      packageCache.set(cacheKey, { data: packages, timestamp: now });
